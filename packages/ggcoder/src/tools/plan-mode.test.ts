...
describe("createExitPlanTool", () => {
  ...
  it("rejects empty plan files", async () => {
    const planPath = path.join(tmpDir, ".mragentix", "plans", "empty.md");
    await fs.writeFile(planPath, "   \n  \n");

    const tool = createExitPlanTool(tmpDir, async () => "should not reach");

    const result = resultToString(
      await tool.execute({ plan_path: ".mragentix/plans/empty.md" }, mockContext),
    );
  ...
  it("rejects non-existent plan files", async () => {
    ...
    const result = resultToString(
      await tool.execute({ plan_path: ".mragentix/plans/ghost.md" }, mockContext),
    );
  ...
  it("calls onExitPlan for valid plans and returns the result", async () => {
    const planPath = path.join(tmpDir, ".mragentix", "plans", "my-plan.md");
    ...
    const result = resultToString(
      await tool.execute({ plan_path: ".mragentix/plans/my-plan.md" }, mockContext),
    );
...
describe("plan mode tool restrictions", () => {
  ...
  describe("write", () => {
    ...
    it("allows writing to .mragentix/plans/ when plan mode is on", async () => {
      ...
      const result = resultToString(
        await tool.execute(
          { file_path: ".mragentix/plans/my-plan.md", content: "# Plan\n\nDo stuff." },
          mockContext,
        ),
      );
      ...
      const written = await fs.readFile(path.join(tmpDir, ".mragentix", "plans", "my-plan.md"), "utf-8");
    ...
    it("creates .mragentix/plans/ directory if it doesn't exist", async () => {
      ...
      await tool.execute(
        { file_path: ".mragentix/plans/auto-created.md", content: "# Plan" },
        mockContext,
      );

      const stat = await fs.stat(path.join(tmpDir, ".mragentix", "plans"));
