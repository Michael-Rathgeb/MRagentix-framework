...
async function loadSkillEntries(cwd: string): Promise<SkillEntry[]> {
  ...
  await loadFromDir(join(cwd, ".mragentix", "skills"), "project", entries);

  // Global skills: ~/.mragentix/skills/*.md
  await loadFromDir(join(homedir(), ".mragentix", "skills"), "global", entries);
      ...
      {loaded && skills.length === 0 && (
        <Box flexDirection="column">
          <Text color={theme.textDim}>
            ...
            {" files to "}
            <Text color={theme.primary}>.mragentix/skills/</Text>
          </Text>
          <Text color={theme.textDim}>
            {"  "}
