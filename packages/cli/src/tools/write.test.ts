...
describe("createWriteTool", () => {
  ...
  it("restricts writes to .mragentix/plans/ in plan mode", async () => {
  ...
  it("allows writing to .mragentix/plans/ in plan mode", async () => {
    const planModeRef = { current: true };
    const tool = createWriteTool(tmpDir, undefined, undefined, planModeRef);

    const raw = await tool.execute(
      { file_path: ".mragentix/plans/plan.md", content: "# My Plan\n" },
    ...
    const written = await fs.readFile(path.join(tmpDir, ".mragentix/plans/plan.md"), "utf-8");
